		...
		<view class="zaiui-bar-search-title-box">
			<view class="cu-bar search bg-white fixed no-shadow">
				...
				<view class="search-form round">
					...
					<input @input="searchInput" v-model="keyword" :adjust-position="false" type="text" placeholder="苹果7" confirm-type="search"/>
					<text class="cuIcon-close close-icon" v-if="search_close" @tap="closeInput"/>
				</view>
				<view class="action">
					<text class="text-red" @tap="doSearch">搜索</text>
				</view>
			</view>
			<!--占位的-->
...
<script>
	...
	export default {
		...
		methods: {
			doSearch(){
				uni.navigateTo({
					url:`/pages/goods/search_result?keyword=${this.keyword}`
				})
			},
			BackPage() {
				uni.navigateBack();

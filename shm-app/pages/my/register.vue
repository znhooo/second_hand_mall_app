<template>
	<view>
		...
		<view class="form re">
			<view class="code">
				<input placeholder="请输入用户名" v-model="userInfo.name" placeholder-style="color: rgba(255,255,255,0.8);"/>
			</view>
			<view class="username">
				<view class="get-code" :style="{'color':getCodeBtnColor}" @click.stop="getCode()">{{getCodeText}}</view>
				<input placeholder="请输入邮箱" v-model="userInfo.email" placeholder-style="color: rgba(255,255,255,0.8);"/>
			</view>
			<view class="code">
				<input placeholder="请输入验证码" v-model="code" placeholder-style="color: rgba(255,255,255,0.8);"/>
			</view>
			<view class="code">
				<input placeholder="请输入手机号" v-model="userInfo.phone" placeholder-style="color: rgba(255,255,255,0.8);"/>
			</view>
			<view class="password">
				<input placeholder="请输入密码" v-model="userInfo.password" password=true placeholder-style="color: rgba(255,255,255,0.8);"/>
			</view>
	...
	export default {
		...
		methods: {
			...
			getCode(){
				...
				if(!(/^\w+@[\da-z\.-]+\.([a-z]{2,6}|[\u2E80-\u9FFF]{2,3})$/.test(this.userInfo.email))){ 
				...
				user.sendEmail(this.userInfo.email).then(res => {
					uni.hideLoading();
					uni.showToast({title: '验证码已发送',icon:"none"});
					this.setTimer();
					this.rightCode = res.data.data
				})
			...
			doReg(){
				...
				user.register(this.userInfo).then(res => {
					uni.hideLoading()
					uni.showToast({
						title:'注册成功!'
					})

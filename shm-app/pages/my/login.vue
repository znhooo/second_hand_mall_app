<template>
	<view>
		...
		<view class="form">
			<view class="username">
				<input placeholder="请输入手机号" v-model="logForm.phone" placeholder-style="color: rgba(255,255,255,0.8);"/>
			</view>
			<view class="password">
				<input placeholder="请输入密码" v-model="logForm.password" password=true placeholder-style="color: rgba(255,255,255,0.8);"/>
			</view>
			<view class="btn" @tap="doLogin">登 录</view>
			<view class="res">
				<view @tap="toPage('/pages/my/register')">用户注册</view>
	...
	export default {
		...
		methods: {
			...
			doLogin(){
				uni.hideKeyboard();
				//验证手机号码
				if(!(/^1(3|4|5|6|7|8|9)\d{9}$/.test(this.logForm.phone))){ 
					// #ifdef APP-PLUS
				...
				user.login(this.logForm).then(res => {
					uni.hideLoading()
					// #ifdef APP-PLUS

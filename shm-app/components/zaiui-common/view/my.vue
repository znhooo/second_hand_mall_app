<template>
	...
	<view class="zaiui-my-box" :class="show?'show':''">
		...
		<view class="bg-gradual-red zaiui-head-box">
			...
			<!--用户信息-->
			<view class="zaiui-user-info-box" v-cloak>
				<!--未登陆-->
				...
				<view class="cu-list menu-avatar" v-else>
					<view class="cu-item">
						<image class="cu-avatar round lg" :src="userInfo.avatar" @load="onoff='1'"></image>
						<view class="content text-xl">
							<view class="text-white" style="height: 10px;">
								<text class="margin-right" v-text="userInfo.name"></text>
							</view>
			...
			<view class="zaiui-user-info-num-box">
				<view class="cu-list grid col-2 no-border">
					<view class="cu-item" @tap="footmarkTap">
						<view class="text-xl" v-if="hasLogin">4</view>
						<view class="text-xl" v-else>-</view>
						<text class="text-sm">足迹</text>
					</view>
					<view class="cu-item">
						<view class="text-xl" v-if="hasLogin">0</view>
						<view class="text-xl" v-else>-</view>
			...
			<!--用户提示-->
			<view class="text-sm zaiui-user-info-tip-box" v-if="!hasLogin" @tap="realNameTap">
		...
		<view class="zaiui-view-content" v-cloak>
			...
			<view class="padding-xs bg-white zaiui-user-info-order-box">
				...
				<view class="cu-list grid col-2 no-border">
					...
					<view class="cu-item" @tap="order_list_tap">
						...
						<view class="text-xxl text-black" v-else>{{buyCount}}</view>
				...
				<view class="grid col-2 money-col">
					<view class="money-item">
						<view class="money-item-view">
							...
							<view class="money-content" @tap="test">
			...
			<view class="padding-xs bg-white margin-top zaiui-user-info-tools-box">
				<view class="padding-sm tools-view">
					<view class="text-black text-bold text-lg tools-title">我的工具</view>
	...
	export default {
		...
		mounted() {
			...
			if(this.userInfo){
				user.login(this.userInfo).then(res => {
					...
					this.userInfo = store.state.userInfo
					good.getSellGoodsCount(this.userInfo.id).then(res => {
						this.sellCount = res.data.data
					})
					good.getBuyGoodsCount(this.userInfo.id).then(res => {
		...
		methods: {
			...
			myReleaseTap(){
				...
				uni.navigateTo({
					url:"/pages/my/myRelease?id="+this.userInfo.id
				})
			},
			//页面被滚动
			...
			order_list_tap() {
				...
				uni.navigateTo({
					url: "/pages/order/list?id="+this.userInfo.id,
					fail(err) {
						console.log(err);
					}
				});

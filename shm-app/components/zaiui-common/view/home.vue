<template>
	<view class="zaiui-home-box" :class="show?'show':''">
		<!--欢迎-->
		<welcome-tip :content="'您好 ' + userInfo.name" :show="true" :c_s="1500" @closeFinish="welcomeClose"/>
...
<script>
	...
	export default {
		name: 'home',
		components: { 
			welcomeTip, swiperBackground, gridMenuList, identifyList, sellQuicklyList, activityList, liveList, videoList, footerTabbar,
			gridSortList, modalImg,goodList
		},
		data() {
			return {
				userInfo:{},
				swiperInfo: {index: 0, show: true, welcome: true, list: []}, 
				headInfo: {Class: "", opacity: 0,},
				goodsShow: true, 
				headTab: {TabCur: 0, scrollLeft: 0, list: []}, 
				viewContent: {welcome: true,}, 
				gridMenuData: [], 
				identifyData: [],
				quickly: {}, 
				activity: [], 
...
//s				console.log(bol);
			...
			goodsTab(e) {
				...
				if(this.goodsTabData.TabCur == 0){
					...
					good.getGoodData(this.current,this.limit).then(res => {
						this.goodsData = res.data.data.data
						this.pages = res.data.data.pages
						uni.hideLoading()
